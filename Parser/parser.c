...
#define _loop1_156_type 1317
#define _loop1_157_type 1318
#define _tmp_158_type 1319
...
// inverse_conjunction: inversion (('nand' inversion))+ | inversion
static expr_ty
inverse_conjunction_rule(Parser *p)
{
    ...
    int _start_col_offset = p->tokens[_mark]->col_offset;
    UNUSED(_start_col_offset); // Only used by EXTRA macro
    { // inversion (('nand' inversion))+
        if (p->error_indicator) {
            D(p->level--);
            return NULL;
        }
        D(fprintf(stderr, "%*c> inverse_conjunction[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "inversion (('nand' inversion))+"));
        expr_ty a;
        asdl_seq * b;
        if (
            (a = inversion_rule(p))  // inversion
            &&
            (b = _loop1_93_rule(p))  // (('nand' inversion))+
        )
        {
        ...
        D(fprintf(stderr, "%*c%s inverse_conjunction[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "inversion (('nand' inversion))+"));
    }
    { // inversion
        if (p->error_indicator) {
            D(p->level--);
...
_loop1_90_rule(Parser *p)
{
    ...
    { // ('or' inverse_disjunction)
        ...
        void *_tmp_147_var;
        while (
            (_tmp_147_var = _tmp_147_rule(p))  // 'or' inverse_disjunction
        )
        {
            _res = _tmp_147_var;
            if (_n == _children_capacity) {
...
_loop1_91_rule(Parser *p)
{
    ...
    { // ('nor' conjunction)
        ...
        void *_tmp_148_var;
        while (
            (_tmp_148_var = _tmp_148_rule(p))  // 'nor' conjunction
        )
        {
            _res = _tmp_148_var;
            if (_n == _children_capacity) {
...
_loop1_92_rule(Parser *p)
{
    ...
    { // ('and' inverse_conjunction)
        ...
        void *_tmp_149_var;
        while (
            (_tmp_149_var = _tmp_149_rule(p))  // 'and' inverse_conjunction
        )
        {
            _res = _tmp_149_var;
            if (_n == _children_capacity) {
...
_loop1_93_rule(Parser *p)
{
    ...
    { // ('nand' inversion)
        ...
        D(fprintf(stderr, "%*c> _loop1_93[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "('nand' inversion)"));
        void *_tmp_150_var;
        while (
            (_tmp_150_var = _tmp_150_rule(p))  // 'nand' inversion
        )
        {
            _res = _tmp_150_var;
            if (_n == _children_capacity) {
        ...
        D(fprintf(stderr, "%*c%s _loop1_93[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "('nand' inversion)"));
    }
    if (_n == 0 || p->error_indicator) {
        PyMem_Free(_children);
        D(p->level--);
...
// _tmp_147: 'or' inverse_disjunction
static void *
...
// _tmp_149: 'and' inverse_conjunction
static void *
